@php
    ...
    $panoramaLocationPicture =
        $headerData['bgImgPath'] ?? ($panorama_location_picture ?? asset('default-panorama.jpg'));
...
<section
    class="custom-header-section section section-no-border section-parallax bg-transparent custom-section-padding-1 custom-position-1 custom-xs-bg-size-cover parallax-no-overflow m-0"
    data-plugin-parallax data-plugin-options="{'speed': 1.5}" data-image-src="{{ url($panoramaLocationPicture) }}">

    <div class="container">
        <div class="row">
            <!-- Hauptbild -->
            <div class="custom-header-img col-lg-4 position-relative custom-sm-margin-bottom-1">
                <img src="{{ url($mainLocationPicture) }}"
                    class="img-fluid custom-border custom-image-position-2 custom-box-shadow-4" alt="Main Image" />
            </div>
            ...
            <div class="col-lg-6 col-xl-5">
                ...
                @if (Auth::guard('admin')->check())
                    <a href="{{ route('verwaltung.header-manager.header_contents.index') }}" target="_blank"
...
<style>
    ...
    .travel-heading-with-bg {
        ...
        display: inline-block;
        margin: 0;
        /* Verhindert zusätzlichen Abstand */
        line-height: 1;
        /* Zeilenhöhe anpassen */
    }
